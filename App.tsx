...
const HomePage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-brand-sand">
      ...
      <section id="work" className="py-20 md:py-32 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16 md:mb-20">
            <div>
              <h2 className="text-4xl md:text-6xl font-black text-slate-900 mb-4 md:mb-6 tracking-tighter">Featured Work</h2>
      ...
      {/* Experience Section */}
      <section id="experience" className="py-20 md:py-32 bg-brand-sand">
        <div className="max-w-7xl mx-auto px-6">
          <div className="mb-16 md:mb-20">
            <h2 className="text-4xl md:text-6xl font-black text-slate-900 mb-4 md:mb-6 tracking-tighter">Experience</h2>
            <p className="text-lg md:text-xl text-slate-500 max-w-2xl">
          ...
          <div className="space-y-8 md:space-y-12">
            {EXPERIENCE.map((exp) => (
              <div key={exp.id} className="bg-white p-8 md:p-12 rounded-[30px] md:rounded-[40px] border-2 border-slate-900 shadow-[6px_6px_0px_0px_rgba(0,0,0,0.2)] hover:shadow-[10px_10px_0px_0px_rgba(0,0,0,0.2)] transition-all">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-6 mb-8">
                  <div>
                    ...
                    <p className="text-lg md:text-xl font-bold text-brand">{exp.role}</p>
                  </div>
                  <div className="text-left md:text-right">
                    <p className="text-base md:text-lg font-bold text-slate-500">{exp.period}</p>
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">{exp.industry}</p>
                  </div>
                </div>
      ...
      <section id="skills" className="py-20 md:py-32 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid sm:grid-cols-2 gap-6 md:gap-8 mb-16 md:mb-20">
            {SKILLS.map((skill) => (
              <div key={skill.name} className="p-6 md:p-8 bg-brand-sand rounded-3xl border-2 border-slate-900 shadow-[4px_4px_0px_0px_rgba(0,0,0,0.3)]">
                <div className="flex items-center gap-4 mb-4">
                  <div className="w-10 h-10 md:w-12 md:h-12 bg-white rounded-xl border-2 border-slate-900 flex items-center justify-center">
                    <Briefcase className="w-5 h-5 md:w-6 md:h-6 text-brand" />
                  </div>
                  <div>
                    <h3 className="text-xl md:text-2xl font-bold text-slate-900">{skill.name}</h3>
                    <p className="text-[10px] md:text-xs font-bold text-slate-400 uppercase tracking-widest">{skill.category}</p>
                  </div>
                </div>
                <p className="text-sm md:text-base text-slate-600 leading-relaxed">{skill.description}</p>
              </div>
            ))}
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-6 md:gap-8">
            {CERTIFICATIONS.map((cert) => (
              <div key={cert.name} className="flex items-center gap-4 md:gap-6 p-6 md:p-8 bg-slate-900 text-white rounded-3xl border-2 border-slate-900 shadow-[4px_4px_0px_0px_rgba(0,0,0,0.5)]">
                <div className="w-12 h-12 md:w-16 md:h-16 bg-brand rounded-2xl flex items-center justify-center flex-shrink-0">
                  <Award className="w-6 h-6 md:w-8 md:h-8 text-white" />
                </div>
                <div>
                  <h4 className="text-lg md:text-xl font-bold mb-1">{cert.name}</h4>
                  <p className="text-sm md:text-base text-slate-400 font-medium">{cert.issuer} • {cert.year}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Joy Section */}
      <section id="joy" className="py-20 md:py-32 bg-brand-sand">
        <div className="max-w-7xl mx-auto px-6">
          <div className="mb-16 md:mb-20">
            <h2 className="text-4xl md:text-6xl font-black text-slate-900 mb-4 md:mb-6 tracking-tighter">Joy</h2>
            <p className="text-lg md:text-xl text-slate-500 max-w-2xl">
          ...
          <div className="grid lg:grid-cols-2 gap-16 md:gap-20 items-center">
            ...
            <div className="relative mt-12 lg:mt-0">
              ...
              <div className="absolute -bottom-4 md:-bottom-10 -left-4 md:-left-10 bg-white p-4 md:p-8 rounded-2xl md:rounded-3xl border-2 border-slate-900 shadow-xl max-w-[200px] md:max-w-xs">
                <p className="text-brand font-black text-lg md:text-2xl mb-1 md:mb-2 italic">"Never guess when you can measure."</p>
                <p className="text-slate-500 font-bold text-xs md:text-base">— Product Philosophy</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section id="contact" className="py-20 md:py-32 bg-slate-900 text-white">
        <div className="max-w-7xl mx-auto px-6 text-center">
          <h2 className="text-4xl md:text-6xl font-black mb-6 md:mb-8 tracking-tighter">Let's build something <span className="text-brand">impactful</span>.</h2>
          <p className="text-xl md:text-2xl text-slate-400 mb-10 md:mb-12 max-w-2xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-8 md:gap-12">
            <a href="https://mail.google.com/mail/?view=cm&fs=1&to=masechaba.c.khoza@gmail.com" target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-lg md:text-2xl font-bold hover:text-brand transition-colors group">
              <div className="p-3 bg-white/10 rounded-2xl group-hover:bg-brand group-hover:text-white transition-all">
                <Mail className="w-6 h-6 md:w-8 md:h-8" />
              </div>
              Email Me
            </a>
            <a href="https://www.linkedin.com/in/masechabakhoza/" target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-lg md:text-2xl font-bold hover:text-brand transition-colors group">
              <div className="p-3 bg-white/10 rounded-2xl group-hover:bg-brand group-hover:text-white transition-all">
